---
layout: default
title: Practical Toolkit
---

# Practical Toolkit

<div class="toolkit-intro">

<p class="toolkit-purpose"><strong>Practical tools for designing, launching, facilitating and sustaining a scientific Community of Practice.</strong></p>

<p>Use these worksheets and canvases to turn the ideas in the guide into a practical plan for your own community.</p>

</div>

## How to use the toolkit

You do not need to complete every tool, and you do not need to use them in a fixed order.

Start with the questions that matter most to your community right now. As your Community of Practice develops, return to the tools and update them.

<div class="toolkit-meta">

<div class="toolkit-meta-box">
<strong>Working online</strong>
Complete the tools directly in your browser and use them as prompts for discussion with your team or community.
</div>

<div class="toolkit-meta-box">
<strong>Working offline</strong>
A downloadable participant workbook will bring the tools together so you can work through them digitally or on paper.
</div>

</div>

---

# Choose the right tool

Think about what you need to do next.

---

## 01 — Define

<div class="toolkit-card">

### Community Blueprint

Bring the key decisions about your Community of Practice together in one place.

Use it to clarify your:

- shared domain and purpose;
- collective value;
- objectives;
- members;
- participation;
- activities;
- communication;
- leadership;
- community rhythm;
- shared practice;
- health and impact.

**[Open the Community Blueprint →](community-blueprint.md)**

</div>

<div class="toolkit-card">

### Member Profile

Understand the people your community is designed to serve.

Use it to explore:

- who your members are;
- why they might join;
- what they need;
- what they can contribute;
- barriers to participation;
- pathways towards deeper involvement.

**[Open the Member Profile →](member-persona.html)**

</div>

---

## 02 — Design

<div class="toolkit-card">

### Participation Canvas

Design multiple ways for members to participate — from observing and asking through to contributing, collaborating and leading.

Use it to think about:

- participation pathways;
- the first 30 days;
- belonging;
- engagement rhythm;
- member contribution.

**[Open the Participation Canvas →](participation-canvas.html)**

</div>

<div class="toolkit-card">

### Governance Canvas

Design a governance model that provides enough structure without unnecessarily centralising control.

Use it to think about:

- coordination;
- decision-making;
- member leadership;
- power and influence;
- community expectations;
- continuity;
- governance review.

**[Open the Governance Canvas →](governance-canvas.html)**

</div>

<div class="toolkit-card">

### Communication & Infrastructure Canvas

Choose the communication channels and technology your community actually needs.

Start with community behaviours rather than platforms.

Use it to plan:

- community behaviours;
- the primary community space;
- additional channels;
- communication norms;
- knowledge management;
- accessibility;
- infrastructure ownership;
- minimum viable infrastructure.

**[Open the Communication & Infrastructure Canvas →](communication-canvas.html)**

</div>

---

## 03 — Launch & Facilitate

<div class="toolkit-card">

### 90-Day Community Launch Plan

Move from preparation to an active Community of Practice.

Use it to plan:

- preparation and member consultation;
- your first interaction;
- follow-up;
- Month 1 — Connect;
- Month 2 — Contribute;
- Month 3 — Co-create;
- your 90-day review.

**[Open the 90-Day Launch Plan →](90-day-launch-plan.html)**

</div>

<div class="toolkit-card">

### Community Activity Design Canvas

Design individual community activities around genuine member needs.

Use it to plan:

- the need;
- participants;
- member contribution;
- activity format;
- facilitation;
- desired outcomes;
- documentation;
- follow-up;
- review.

**[Open the Activity Design Canvas →](activity-design-canvas.html)**

</div>

---

## 04 — Sustain & Evaluate

<div class="toolkit-card">

### Community Ownership Canvas

Strengthen distributed responsibility, member leadership and succession.

Use it to explore:

- who currently holds responsibility;
- opportunities for contribution;
- transferring responsibility;
- leadership development;
- succession;
- inclusion and access to leadership;
- recognition;
- community ownership.

**[Open the Community Ownership Canvas →](community-ownership-canvas.html)**

</div>

<div class="toolkit-card">

### Community Evaluation Canvas

Evaluate whether your Community of Practice is creating meaningful value.

Use it to explore:

- what you want to learn;
- your evidence chain;
- participation;
- engagement;
- relationships;
- learning and change in practice;
- collective value;
- unexpected outcomes;
- qualitative evidence;
- core indicators;
- baseline information;
- community health;
- what should happen next.

**[Open the Community Evaluation Canvas →](community-evaluation-canvas.html)**

</div>

---

# A suggested journey

The tools can be used independently, but the following sequence can help if you are starting a new community.

<div class="toolkit-journey">

<div class="toolkit-journey-step">

<strong>1. Define</strong>

<br>

Community Blueprint · Member Profile

</div>

<div class="toolkit-journey-step">

<strong>2. Design</strong>

<br>

Participation Canvas · Governance Canvas · Communication & Infrastructure Canvas

</div>

<div class="toolkit-journey-step">

<strong>3. Launch</strong>

<br>

90-Day Community Launch Plan

</div>

<div class="toolkit-journey-step">

<strong>4. Facilitate</strong>

<br>

Community Activity Design Canvas

</div>

<div class="toolkit-journey-step">

<strong>5. Build ownership</strong>

<br>

Community Ownership Canvas

</div>

<div class="toolkit-journey-step">

<strong>6. Evaluate</strong>

<br>

Community Evaluation Canvas

</div>

<div class="toolkit-journey-step">

<strong>7. Adapt</strong>

<br>

Return to whichever tool needs to change.

</div>

</div>

This is not a rigid process. Communities develop iteratively, and you may need to move backwards and forwards between tools.

---

<div class="toolkit-download">

<h2>Download the complete toolkit</h2>

<p><strong>Prefer to work offline?</strong></p>

<p>A complete <strong>Participant Workbook & Toolkit</strong> will bring all of these tools together in one document.</p>

<p>You will be able to:</p>

<ul>
<li>download the complete toolkit;</li>
<li>work through it on your own computer;</li>
<li>print individual sections;</li>
<li>use it with a founding group or community;</li>
<li>return to your plans as the community develops.</li>
</ul>

<p><strong>The downloadable workbook will be available here soon.</strong></p>

</div>

---

# Tips for using the toolkit

### Work with other people

These tools are often most useful when completed collaboratively. Use them to start conversations rather than simply to document decisions.

### Do not aim for perfect answers

Your community will change as you learn more. Treat the tools as living documents.

### Start with the real need

Especially when designing activities or infrastructure, begin with what members need to do rather than what you can offer them.

### Look for member contribution

A Community of Practice depends on interaction between members. Ask regularly what members are contributing, not only what they are receiving.

### Revisit the tools

The same canvas may look different six months after launch. That is a sign that the community is developing.

---

## Return to the guide

**[← Back to the Community of Practice guide](../index.html)**
