---
layout: default
title: Community Evaluation Canvas
---

# Community Evaluation Canvas

<div class="toolkit-intro">

<p class="toolkit-purpose"><strong>What this tool is for:</strong> Use this canvas to evaluate whether your Community of Practice is creating meaningful value.</p>

<p>The aim is not to measure everything. Instead, use a small number of meaningful indicators to understand participation, engagement, relationships, learning, changes in practice and wider outcomes.</p>

</div>

<div class="toolkit-meta">

<div class="toolkit-meta-box">
<strong>Use this when</strong>
You are reviewing the health, value or impact of an established community.
</div>

<div class="toolkit-meta-box">
<strong>Work on it</strong>
Periodically with organisers and, where possible, community members.
</div>

</div>

## How to use this tool

1. Start with the questions you want to answer.
2. Think about how community activity could lead to meaningful outcomes.
3. Combine quantitative and qualitative evidence.
4. Choose a small number of useful indicators.
5. Review the evidence and decide what the community should do next.

---

# 1. What do we want to learn?

Before choosing indicators, identify the questions that matter.

### Are people participating?

<div class="response-area"></div>

### Are members learning from one another?

<div class="response-area"></div>

### Are useful relationships developing?

<div class="response-area"></div>

### Is the shared practice evolving?

<div class="response-area"></div>

### Are members applying what they learn?

<div class="response-area"></div>

### Is the community creating opportunities for collaboration?

<div class="response-area"></div>

### Is the community influencing practice beyond its membership?

<div class="response-area"></div>

### Is the community becoming more sustainable?

<div class="response-area"></div>

---

# 2. Map your evidence chain

Think about how community activity could lead to meaningful outcomes.

<div class="prompt">

<strong>Community activity</strong>

<br>

↓

<br>

<strong>Member interaction</strong>

<br>

↓

<br>

<strong>Learning / relationship</strong>

<br>

↓

<br>

<strong>Change in practice</strong>

<br>

↓

<br>

<strong>Wider outcome</strong>

</div>

## Describe your own evidence chain

### Community activity

<div class="response-area"></div>

### Member interaction

<div class="response-area"></div>

### Learning or relationship

<div class="response-area"></div>

### Change in practice

<div class="response-area"></div>

### Wider outcome

<div class="response-area large"></div>

---

# 3. Participation

Track patterns rather than relying only on total membership.

| Indicator | Current | Previous period | Change |
|---|---:|---:|---:|
| Registered members | | | |
| Active members | | | |
| Returning members | | | |
| Members contributing | | | |
| Member-led activities | | | |
| New members participating | | | |
| Members taking leadership roles | | | |

### What do these patterns tell us?

<div class="response-area large"></div>

---

# 4. Engagement

What are members actually doing?

Consider:

- asking questions
- answering others
- sharing experience
- sharing resources
- solving problems
- making introductions
- organising activities
- collaborating
- supporting newer members
- taking leadership roles

### What forms of engagement are we seeing?

<div class="response-area large"></div>

### Which forms are missing?

<div class="response-area large"></div>

---

# 5. Relationships

Assess whether useful relationships are developing.

### Have members developed new useful relationships?

- [ ] Yes
- [ ] No
- [ ] Not sure

### Are members contacting one another outside organised activities?

- [ ] Yes
- [ ] No
- [ ] Not sure

### Are members seeking advice from one another?

- [ ] Yes
- [ ] No
- [ ] Not sure

### Are members helping one another?

- [ ] Yes
- [ ] No
- [ ] Not sure

### Are new collaborations emerging?

- [ ] Yes
- [ ] No
- [ ] Not yet

### What is the strongest evidence of relationship-building?

<div class="response-area large"></div>

---

# 6. Learning and change in practice

### What are members learning?

<div class="response-area large"></div>

### What are members doing differently?

<div class="response-area large"></div>

### What new practices, skills or approaches are emerging?

<div class="response-area large"></div>

### Are members sharing their learning beyond the community?

<div class="response-area"></div>

---

## Example of changed practice

### Before participating in the community, I...

<div class="response-area"></div>

### Through the community, I learned...

<div class="response-area"></div>

### As a result, I changed...

<div class="response-area"></div>

### This change has led to...

<div class="response-area large"></div>

---

# 7. Collective value

Some outcomes become possible because people are connected.

Look for:

- collaborative research
- joint funding applications
- shared training
- shared resources
- new partnerships
- joint publications
- mentoring relationships
- cross-institutional projects
- collective responses to challenges

### What became possible because these people were connected?

<div class="response-area large"></div>

---

# 8. Unexpected outcomes

Not all valuable outcomes can be predicted in advance.

### What happened that we did not plan for?

<div class="response-area large"></div>

### What new opportunities emerged?

<div class="response-area"></div>

### What new relationships or collaborations emerged?

<div class="response-area"></div>

### What unexpected learning occurred?

<div class="response-area"></div>

---

# 9. Qualitative evidence

Collect stories that help explain why the community matters.

Useful evidence might include:

- member testimonials
- short case studies
- examples of changed practice
- examples of collaboration
- examples of problems solved
- examples of resources being reused

## Member story

### What happened?

<div class="response-area large"></div>

### Who was involved?

<div class="response-area"></div>

### What changed?

<div class="response-area"></div>

### What role did the community play?

<div class="response-area large"></div>

### What might not have happened without the community?

<div class="response-area large"></div>

---

# 10. Choose your core indicators

Choose no more than **5–10 core indicators**.

| What do we want to understand? | Indicator | Data source | Frequency |
|---|---|---|---|
| Participation | | | |
| Engagement | | | |
| Relationships | | | |
| Learning | | | |
| Change in practice | | | |
| Collaboration | | | |
| Wider outcomes | | | |
| Sustainability | | | |

### Why did we choose these indicators?

<div class="response-area large"></div>

---

# 11. Establish a baseline

If possible, record the situation before or at the beginning of the community.

### Existing collaborations

<div class="response-area"></div>

### Existing knowledge-sharing practices

<div class="response-area"></div>

### Existing relationships

<div class="response-area"></div>

### Current training or learning activity

<div class="response-area"></div>

### Existing resources

<div class="response-area"></div>

### Current challenges

<div class="response-area large"></div>

### Other relevant baseline information

<div class="response-area"></div>

---

# 12. Review community health

Complete this periodically.

| Area | Healthy | Needs attention | Evidence |
|---|:---:|:---:|---|
| Shared purpose remains relevant | ☐ | ☐ | |
| Members interact regularly | ☐ | ☐ | |
| Members help one another | ☐ | ☐ | |
| Members share experience | ☐ | ☐ | |
| New members can participate | ☐ | ☐ | |
| Experienced members support others | ☐ | ☐ | |
| Members lead activities | ☐ | ☐ | |
| Relationships are developing | ☐ | ☐ | |
| Shared practice is evolving | ☐ | ☐ | |
| Members see value in participating | ☐ | ☐ | |
| Community infrastructure is adequate | ☐ | ☐ | |
| Responsibility is distributed | ☐ | ☐ | |
| Evidence of outcomes exists | ☐ | ☐ | |

### What stands out from this health check?

<div class="response-area large"></div>

---

# 13. Decide what happens next

Use your evidence to decide whether the community should:

### Continue

The community is creating value and should continue broadly as it is.

### Adapt

The community is valuable but something needs to change.

### Refocus

The original purpose is no longer sufficient.

### Pause

There is currently insufficient value or capacity to continue.

### Close

The community has fulfilled its purpose or is no longer the appropriate mechanism.

---

## Decision

<div class="response-area"></div>

## Why?

<div class="response-area large"></div>

## What will we change?

<div class="response-area large"></div>

## When will we review this again?

<div class="response-area"></div>

---

<div class="toolkit-output">

<h2>Your output</h2>

<p>By completing this canvas, you should have:</p>

<ul>
<li>clear questions about what you want to learn;</li>
<li>an evidence chain connecting activity to outcomes;</li>
<li>a small set of useful indicators;</li>
<li>quantitative and qualitative evidence;</li>
<li>a baseline where appropriate;</li>
<li>an assessment of community health;</li>
<li>a decision about what should happen next.</li>
</ul>

<p>The aim is not to prove that every outcome was caused by the community. Instead, use evidence to understand whether the community is creating meaningful value and what you should learn or change next.</p>

</div>

<div class="toolkit-next">

<h2>Next step</h2>

<p>Use the findings to adapt your community.</p>

<p>Return to the <strong>Community Blueprint</strong>, <strong>Participation Canvas</strong>, <strong>Community Ownership Canvas</strong> or other relevant tools when the evaluation identifies something that needs to change.</p>

</div>

---

## Related section

**[Evaluate community health, value and impact →](../10-evaluate-community-health.html)**

Return to the guide for practical guidance on evaluating participation, relationships, learning, outcomes and community health.

---

## Sources & foundations

### Wenger-Trayner & Wenger-Trayner

Wenger-Trayner, E. & Wenger-Trayner (2015). *Introduction to Communities of Practice: A Brief Overview of the Concept and its Uses.*

This source informs the emphasis on assessing CoP value through a combination of quantitative and qualitative evidence, while recognising that attribution of outcomes may be complex.

It also provides examples of community activities and interactions that can be used to identify meaningful engagement.

### The Less Visible Leader

Robinson, A. (2012). *The Less Visible Leader: Emerging Leadership Models for Environmental Networks, Coalitions, and Collaboratives*. Institute for Conservation Leadership.

This source informs the emphasis on assessing relationships and network strength rather than relying only on participation counts.

### Our synthesis

The Community Evaluation Canvas, including the evidence chain, indicator-selection table and Community Health Check, was developed specifically for this guide.

It is an original practical synthesis rather than a reproduction of any of the sources above.
